  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && typeof parsed === 'object') {
        adjState = { ...adjState, ...['brightness','contrast','saturation','hue'].reduce((acc,k)=>{
    ...
    function persistAdjustments() {
      try { localStorage.setItem(ADJ_LS_KEY, JSON.stringify(adjState)); } catch(_){}
    }
    function updateAdjUI(fromLoad=false) {
      ...
      if(!fromLoad) { applyVideoAdjustments(); persistAdjustments(); }
    }
    function wireAdjustments() {
      const hook = (el, key) => {
        if(!el) return; if (el._wired) return; el._wired = true;
        el.addEventListener('input', () => { let val = parseFloat(el.value); if(!isFinite(val)) return; adjState[key]=val; updateAdjUI(); });
        el.addEventListener('change', () => { let val = parseFloat(el.value); if(!isFinite(val)) return; adjState[key]=val; updateAdjUI(); });
      };
    ...
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', wireAdjustments); else wireAdjustments();
        ...
        function init() {
          ...
          items.forEach((it, idx) => {
            ...
            hdr.addEventListener('click', (e) => {
              if (hdr._skipNextClick) { hdr._skipNextClick = false; return; }
        ...
        const Player = (() => {
                ...
                try {
                  ...
                  if (!immediateShown) {
                    ...
                    try {
                      ...
                      const fi = document.getElementById('fiThumbnail'); if (fi) fi.src = fresh;
                      const card = document.querySelector(`.card[data-path="${activePath.replace(/"/g,'\\"')}"]`);
                      if (card) { 
                        const img = card.querySelector('img.thumb'); 
                    ...
                    try {
                      ...
                      while (Date.now() < deadline) {
                        ...
                        if (hr.ok) {
                          ...
                          const fi = document.getElementById('fiThumbnail'); if (fi) fi.src = finalUrl;
                          const card = document.querySelector(`.card[data-path="${activePath.replace(/"/g,'\\"')}"]`);
                          if (card) { const img = card.querySelector('img.thumb'); if (img) img.src = finalUrl; }
                          break;
                        }
                    ...
                    try {
                      ...
                      const card = document.querySelector(`.card[data-path="${activePath.replace(/"/g,'\\"')}"]`);
                      if (card) { const img = card.querySelector('img.thumb'); if (img) img.src = fresh; }
                    } catch(_){}
                  }
                  ...
                  if (got) {
                    try {
                      ...
                      setTimeout(()=>{ if(card) card.classList.remove('thumb-updated'); }, 1200);
                    } catch(_){}
                  } else {
                    setTimeout(() => loadLibrary(), 250);
                  }
                } catch(e) { notify('Failed to set thumbnail', 'error'); }
              });
            }
            ...
            if (volSlider && !volSlider._wired) {
              ...
              volSlider.addEventListener("input", () => {
                ...
                if (v > 0) {
                  ...
                  if (videoEl.muted) videoEl.muted = false; // unmute when user drags up
                }
                syncControls();
              });
            }
          ...
          function syncControls() {
            try {
              ...
              if (volSlider && typeof videoEl.volume === "number") {
                ...
                volSlider.title = pct + '%';
                if (videoEl.muted || v === 0) volSlider.classList.add('is-muted'); else volSlider.classList.remove('is-muted');
              }
          ...
          async function refreshSidebarThumbnail(path) {
            try {
              ...
              if (!fiThumbWrap || !fiThumbImg || !path) return;
              let updated = false;
          ...
          try { window.refreshSidebarThumbnail = refreshSidebarThumbnail; } catch(_){}
          ...
          function _maybeRefreshThumb() { const p = (window.Player && typeof window.Player.getPath==='function') ? window.Player.getPath() : (typeof currentPath==='function'?currentPath():currentPath); if (p) refreshSidebarThumbnail(p); }
          ...
          function open(path) {
            ...
            currentPath = path;
            try { if (typeof refreshSidebarThumbnail === 'function') refreshSidebarThumbnail(currentPath); } catch(_){}
            ...
            if (clearBtn && !clearBtn._wired) {
              ...
              clearBtn.addEventListener("click", async () => {
                try {
                  ...
                  if (removed === 0) clearBtn.style.display = 'none';
                } catch (e) {
            ...
            async loadCoverage() {
              try {
                ...
                if (data.status === "success") {
                  ...
                  try { if (currentPath) refreshSidebarThumbnail(currentPath); } catch(_){}
                }
              }
              catch (error) {
